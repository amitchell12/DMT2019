...
###### PEAK ACCELERATION #####
PAX_medians <- aggregate(PAX ~ PPT * VIEW * SIDE * DOM * POSITION * ECC * SITE * GRP * DIAGNOSIS, 
                           median, data = res2)
# remove 310, not using
PAX_medians <- PAX_medians[PAX_medians$PPT != 310 ,]
# data points from 407 & 402 (abnormally large)
PAX_medians <- PAX_medians[!(PAX_medians$PPT == '407' & PAX_medians$PAX > 200000) ,]
PAX_medians <- PAX_medians[!(PAX_medians$PPT == '402' & PAX_medians$PAX > 100000) ,]

PAX_means <- aggregate(PAX ~ PPT * VIEW * SIDE * DOM * SITE * GRP * DIAGNOSIS,
                         mean, data = PAX_medians)
# average across side
PAXav <- aggregate(PAX ~ PPT * VIEW * SITE * GRP * DIAGNOSIS,
                     mean, data = PAX_medians)
...
ggplot(PAXsummary, aes(x = VIEW, y = PAX, colour = DIAGNOSIS, group = DIAGNOSIS,
                         shape = DIAGNOSIS)) +
  geom_point(size = 3, position = position_dodge(width = .3)) +
  geom_line(aes(group = DIAGNOSIS), size = 0.5, alpha = .5,
            position = position_dodge(width = .3)) +
