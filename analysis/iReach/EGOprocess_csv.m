% egocentric iReach ipad task processing code for csv files  
% 15/7/20
...
% other file
[FileName,PathName,FilterIndex] = uigetfile('.csv','***Pick _TOUCH file***'); % user load main filename
...
a=strfind(TXT(:,13),'false');% find real trials
for i=1:length(a)
...
for trial=1:nTrials
    ...
    trialData(2:end,15)= s>50;  %(50mm per sec threshold for onset / offset)
    trialData(2:end,16)=s; %% store actual velocity
    
    f2=find(trialData(:,15)==1); %% movements > threshold
    ...
    targetPos(trial,1:2)=trialData(f2(1),10:11); %% target position
    
    traj{trial}=[trialData(f2(1):endInd,[6 8 9 10 11])];  %% store trajectories
...
% calculate accuracy?
Acc=targetPos(:,1:2)-moveData(:,3:4); %% target - end position
...
hold, plot(xx(:,1),xx(:,2),'b*') % markers for target positions
title('End Position against possible target positions')
...
figure, scatter(targetPos(:,1),AE), title('Absolute error against x position');  %% scatter of AE
print('-dtiff',sprintf('%s_Absolute error.tiff',subject_code));

% save things?
outname=[subject_code '_' FileName2(1:end-4) '_processed.mat'];
outData=[targetPos reactionTime moveT' trajL' moveData(:,3:6) Acc AE];
%% xtarget, Ytarget,RT, MT, trajL, x,y, PV, TPV,  xerror, yerror, AE

xlswrite([subject_code '_EGO_processed.xlsx'],outData);

save(outname,'outData','moveData','traj','trajL','targetPos','moveT','reactionTime','nTrials','Acc','AE');
